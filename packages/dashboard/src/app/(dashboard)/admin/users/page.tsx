...
const SORTABLE_COLUMNS: { field: SortField; label: string; align?: string }[] = [
  ...
  { field: 'balance', label: '余额', align: 'text-right' },
  { field: 'rateMultiplier', label: '费率', align: 'text-right' },
...
export default function UsersPage() {
  ...
  async function handleEditSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setEditMsg(err instanceof Error ? err.message : '操作失败');
    }
  }
  ...
  async function handleToggleSuspend(user: UserInfo) {
    try {
      if (user.isSuspended) {
        ...
        // 暂停：通过 set-spending-limit 触发？这里直接调 unsuspend 的反向没有现成 API
        // 实际上我们只需要 unsuspend，suspend 由系统自动触发
        // 但管理员可能需要手动暂停，后面可以加
        await api.unsuspendUser(user.userId);
      }
      loadUsers();
  ...
  return (
    <div>
      ...
      <Dialog open={editDialogOpen} onOpenChange={setEditDialogOpen}>
        ...
        <DialogPopup>
          ...
          <form id="user-edit-form" onSubmit={handleEditSubmit} className="grid grid-cols-2 gap-4 px-6">
            ...
            <div>
              <label className="block text-xs text-muted-foreground mb-1">费率倍率</label>
              <Input
                type="number"
                step="0.01"
                min="0"
                ...
                required
              />
              <p className="text-xs text-muted-foreground mt-1">1 = 标准费率，1.1 = 加价 10%，0.9 = 优惠 10%</p>
            </div>
          </form>
      ...
      <div className="relative mb-4 max-w-sm">
        ...
        <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="搜索邮箱..." className="pl-9" />
      </div>
          ...
          {sortedUsers.length > PAGE_SIZE && (
            <div className="flex items-center justify-between mt-4">
              <span className="text-sm text-muted-foreground">
                共 {sortedUsers.length} 条，第 {safePage}/{totalPages} 页
              </span>
