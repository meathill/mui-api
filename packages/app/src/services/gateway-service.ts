 ...
 * 所有 provider 均经 Gateway Stored Keys 托管凭证，Worker 仅注入 cf-aig-authorization。
 *
...
export class GatewayService {
  ...
  async proxyNative(provider: string, path: string, request: Request): Promise<Response> {
    ...
    // 网关认证：全部经 Stored Keys 托管，Worker 仅注入 cf-aig-authorization
