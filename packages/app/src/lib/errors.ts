...
export function badRequest(c: Context, message: string, details?: unknown) {
  return c.json(createErrorResponse(message, ErrorTypes.INVALID_REQUEST, undefined, details), 400);
}
...
export function internalError(c: Context, message: string = '内部错误', details?: unknown) {
  return c.json(createErrorResponse(message, ErrorTypes.INTERNAL_ERROR, undefined, details), 500);
}
