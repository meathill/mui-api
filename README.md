...
- **统一入口**：对外暴露 OpenAI 兼容的 `/v1/*` 接口（chat / images / models）与 MCP server，按模型把请求分发到 OpenAI、Google、Anthropic、Workers AI、Xiaomi MiMo 等供应商。
- **凭证集中**：调用方只持有本服务的 API Key（`sk-gw-` 前缀）或 OAuth 2.0 token，真实 Provider Key 由 CF AI Gateway 统一托管。
